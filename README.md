...
本项目基于沪深300ETF华泰柏瑞（510300）的日频历史数据，构建并验证了一个基础双均线择时策略。正式研究区间为 2012-05-28 至 2025-12-31，固定使用 20 日和 60 日移动平均线，并假设单边交易成本为 10 bps。

在数据处理方面，项目使用不复权价格，并单独合并现金分红数据构造总收益率，从而避免将除息日的机械价格下跌错误识别为真实投资损失。买入持有基准和策略收益均采用相同的总收益口径。

为避免未来数据泄漏，策略使用前一交易日生成的信号决定当日持仓，即通过：
...
在固定参数下，全样本策略年化收益约为 1.95%，低于买入持有基准的 6.38%；策略夏普比率约为 0.20，也低于基准的 0.39。策略最大回撤约为 -48.03%，并未优于基准的 -45.44%。

因此，从全样本结果看，该均线策略没有表现出稳定优于买入持有基准的能力。

样本内与样本外结果存在明显差异。

样本内策略年化收益约为 5.53%，夏普比率约为 0.40；样本外策略年化收益下降至约 -3.95%，夏普比率下降至约 -0.28。样本外区间归一化净值最终为 0.8164，说明样本内观察到的正收益没有稳定延续到后续市场阶段。

参数敏感性分析显示，不同均线参数组合的表现并不稳定。部分参数在样本内表现较好，但样本外表现明显恶化；样本内表现较好并不意味着样本外表现也较好。

同时，较短周期参数通常产生更高换手率，因此更容易受到交易成本影响。不能根据样本外结果重新挑选所谓“最优参数”，否则容易引入数据窥探和过拟合。

交易成本敏感性分析显示，随着单边成本从 0 bps 提高至 20 bps，策略年化收益和夏普比率持续下降。换手率由交易信号决定，不随成本参数变化，但累计成本扣减会随单位交易成本近似线性增加。

这说明即使是日频、相对低频的基础策略，也不能忽略交易成本对最终结果的影响。

综合来看，本项目不能证明双均线策略对沪深300ETF具有稳定的预测能力，也不能证明其能够持续获得超额收益。策略在不同市场阶段和不同参数下表现差异较大，样本外结果尤其说明其稳健性有限。

本项目的主要价值不在于策略表现，而在于建立了一个口径明确、逻辑可解释、结果可复现的基础量化回测框架。项目完整处理了：
...
当前研究仍存在单一标的、单一策略、参数范围有限、未建立逐笔交易账本、未考虑滑点与市场冲击、未进行滚动样本检验等局限。

因此，现有结果应被视为对回测流程和研究规范的验证，而不是对策略未来盈利能力的承诺。
